package com.lrz.eshop.mapper.chat;
...
public interface MessageMapper extends BaseMapper<Message> {
    ...
    @Results({
        @Result(property = "messageContents", column = "id", javaType = List.class,
                many = @Many(select = "com.lrz.eshop.mapper.chat.MessageContentMapper.selectByMessageId")
        ),
        @Result(property = "user", column = "user_id", javaType = UserSocialInfo.class,
                many = @Many(select = "com.lrz.eshop.mapper.user.UserInfoMapper.getUserSocialInfo")
        ),
    })
